...
public class Spectra {
  ...
  private static Map<Class<? extends Annotation>, List<Method>> scanAnnotations(Class<?> type) {
    Map<Class<? extends Annotation>, List<Method>> annotations = new HashMap<>();
    Class<?> current = type;

    //TODO: mudar aqui para validar que cada classe possui somente um método anotado com warmup e cooldown
    while (current != null && current != Object.class) {
      for (Method method : current.getDeclaredMethods()) {
        for (Annotation ann : method.getAnnotations()) {
          //TODO: Validar também as anotações que são recuperadas em getAnnotations()
          annotations
            .computeIfAbsent(ann.annotationType(), __ -> new ArrayList<>())
            .add(method);
        }
      }
